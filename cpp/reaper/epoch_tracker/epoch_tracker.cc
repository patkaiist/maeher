...
//
// Modified by the maeher project (https://github.com/patkaiist/maeher):
// removed two diagnostic fprintf(stdout, ...) calls in ComputeFeatures().
// As a shared library this code may be called concurrently from multiple
// threads, where writing to the process-wide stdout is both a thread-safety
// hazard and unwanted console clutter; removing the writes avoids both.
// No algorithmic changes were made and numerical output is unchanged.
...
bool EpochTracker::ComputeFeatures(void) {
  ...
  float mean = 0.0;
  GetSymmetryStats(residual_, &positive_rms_, &negative_rms_, &mean);
  // maeher: upstream printed residual-symmetry diagnostics to stdout here.
  // Removed so the library is thread-safe (no concurrent writes to the
  // process-wide stdout) and does not clutter the console on every call.
  // Numerical output is unaffected.
  if (positive_rms_ > negative_rms_) {
    for (size_t i = 0; i < residual_.size(); ++i) {
